...
截至 2026-07-21，Stand 已完成数据整理、奖励修复、无跳变初始化和三阶段正式训练。`scripts/train_g1_armhack_stand.sh` 已从 S3 G1 `model_9996.pt` 完成第一阶段；`scripts/train_g1_armhack_stand_randomized_payload.sh` 完成第二阶段的范围内随机姿态、minimum-jerk 插值轨迹和腕部末端负载训练；`scripts/train_g1_armhack_stand_robust.sh` 又完成第三阶段的更大摔倒惩罚、torso 外力和关节参数随机化训练。HEC-5090 的第三阶段最终模型已下载到本机 `../checkpoint/stand/model_2999.pt`，当前 SHA-256 为 `146aca1f547ce073756c942508e8ea43c8cea91b27eee3b8347dd4131c87bc5f`。针对真机初始姿态应为双臂自然下垂、而旧训练分布没有保证这一点的问题，现已新增第四阶段 `StandDownToDefault` 续训任务和脚本，并完成真实 Isaac smoke；完整正式续训尚未启动。历史第二阶段模型及其报告仍保留，不能只凭同名 `model_2999.pt` 判断来源，必须同时核对完整路径和 SHA。
...
LeggedLab-Isaac-AMP-G1-StandDownToDefault-v0
...
- 第二阶段正式 run 为 `2026-07-15_14-12-54_armhack_stand_randomized_payload_from_model2999_full_20260715`，其 `model_2999.pt` SHA-256 为 `877e929d516cffe9131cc235477ceef4b226ec69e41c0f1c23e48816cfa28821`；它仍作为第三阶段输入和历史对照保留，不再是当前默认测试模型；
...
- 第三阶段最终配置先通过 `8 env × 1 iteration` smoke，随后已在 HEC-5090 完成 `4096 env × 3000 iteration` 正式训练；最终 `model_2999.pt` 已保存到本机 `../checkpoint/stand/model_2999.pt`，大小 `14,825,781 bytes`，SHA-256 为 `146aca1f547ce073756c942508e8ea43c8cea91b27eee3b8347dd4131c87bc5f`；
- 第四阶段入口从上述第三阶段 checkpoint 做 policy-only 续训；每次 reset 把双臂实际关节状态直接写到共享预设 `AD_natural_down`，再用每环境独立采样的 `3–9 s` minimum-jerk 速度同时抬到 `P0_symmetric_reference`。`8 env × 1 iteration` Isaac smoke 已跑通完整 `AD→P0`、PPO 更新与 checkpoint 保存；
...
- 当前正式 Stand actor 已导出为 `../checkpoint/stand/stand_robust_model_2999.onnx`，并生成零速度部署元数据；96 维输入、29 维输出、Unitree motor 映射、双臂动作覆盖和手指/夹爪独立控制边界见 `docs/armhack_stand_real_deployment.md`；
...
这里的 smoke `model_0.pt` 只证明任务注册、数据加载、Isaac Sim、rollout、PPO 更新和保存执行链可运行，不代表一轮更新已经学会新任务。第四阶段的 smoke checkpoint 也不能用于真机部署。完成正式续训和固定测试前，当前测试、可视化和部署仍应显式指向本机归档的第三阶段模型 `../checkpoint/stand/model_2999.pt`；显式传入 `CHECKPOINT="$STAND_CKPT"` 并在报告中保留 SHA，才能与历史两个同名 `model_2999.pt` 区分。
...
bash -n scripts/train_g1_armhack_stand_down_to_default.sh
...
当前预期结果为数据检查 `[PASS]`、静态测试 `12 passed`。`PYTEST_DISABLE_PLUGIN_AUTOLOAD=1` 用于隔离 `~/.local` 中与 Conda 无关的 pytest 插件冲突；训练入口本身不依赖该设置。
...
### 1.1.7.2 默认姿态→双臂前伸→收回→自然下垂专项测试

新增确定性专项模式 `MODE=default_forward_return_down`，固定文件为：
...
CSV 共 `1976` 帧、`39.5 s`、50 Hz，表头严格为 `time_s + 14 个双臂关节`，不包含根节点、腰和腿。四个目标状态与三段过渡为：

| 时间 | 曲线标记 | 动作 |
|---:|---|---|
| 0–5.0 s | `P0` | 双臂保持 S3 G1 默认姿态 |
| 5.0–11.5 s | `P0→F` | 双臂同时、平滑地向前伸直 |
| 11.5–16.5 s | `AH` | 双臂向前、手腕约与肩同高并保持 |
| 16.5–23.0 s | `F→P0` | 双臂同时收回默认姿态 |
| 23.0–28.0 s | `P0R` | 收回后的默认姿态保持 |
| 28.0–34.5 s | `P0→AD` | 双臂同时移动到自然下垂姿态 |
| 34.5–39.5 s | `AD` | 自然下垂保持 |

默认姿态来自 `UNITREE_G1_29DOF_CFG.init_state` 和 MuJoCo `g1_amp.yaml` 的同一组默认角：左臂 `[0.30, 0.25, 0, 0.97, 0.15, 0, 0]`，右臂 `[0.30, -0.25, 0, 0.97, -0.15, 0, 0] rad`。前伸目标沿用已做过腕肩高度检查的左臂 `72.238928 s`、右臂 `323.462679 s` 实测 7-DoF 组合；自然下垂目标沿用原始手臂数据 `404.897585 s` 的完整双臂姿态。三段过渡均为 6.5 s 五次 minimum-jerk；其中默认到前伸按逐关节限速计算至少需要 `6.171975 s`，因此当前设置留有余量。该专项动作不并入默认 `MODE=all`。

新终端用 IsaacLab GUI 查看：
...
```bash
CHECKPOINT="$STAND_CKPT" MODE=default_forward_return_down PAYLOAD_KG=0 \
HEADLESS=True REAL_TIME=False MAX_STEPS=1975 \
bash scripts/vis_g1_armhack_stand_eval.sh

CHECKPOINT="$STAND_CKPT" MODE=default_forward_return_down PAYLOAD_KG=0 \
HEADLESS=True REAL_TIME=False MAX_STEPS=1975 \
bash scripts/vis_g1_armhack_stand_eval.sh --video --video_length 1975
```
...
2026-07-21 已用当前第三阶段 `model_2999.pt`（SHA 前缀 `146aca1f547c`）完整执行 MuJoCo 无负载测试：`1976` 个控制样本全部播放，`healthy=True`、无摔倒、最低 root 高度 `0.785 m`；torso 水平位移 RMS/最大值为 `0.03319/0.04960 m`，pitch 位移 RMS/最大值为 `0.05563/0.11148 rad`，双臂跟踪 MAE/RMS/最大值为 `0.03573/0.05246/0.17713 rad`。因此该模型通过了本动作的最低生存稳定性检查，但最大 pitch 约 `6.39°`，不能表述为高精度躯干稳定。报告和阶段曲线位于：

```text
checkpoint/stand/Test Reports/StandArmOnlyMuJoCo/
  model_2999_146aca1f547c__mujoco__default_forward_return_down__payload_0kg.md
  model_2999_146aca1f547c__mujoco__default_forward_return_down__payload_0kg__torso_world_6d.png
```
...
`MODE` 支持 `all`、六类确定性集合，`representative_pose`/`synthesized_pose`/`randomized_pose`、三类 `*_trajectory` 的单项模式、无需 `ITEM` 的 `down_to_horizontal` 专项模式，以及下文新增的 `generated_random_trajectory`。确定性单项模式必须同时指定合法 `ITEM`；新随机模式不使用 `ITEM`。若在 GUI 中途手工关闭窗口，rollout 会提前结束，并在报告中得到 `complete_csv_playback=False`，不能当作完整通过。

#### 1.1.9.1 从 HEC-5090 同步脚本并做离线验证（2026-07-20）

本次按用户指定路径下载了 HEC-5090 上的两个脚本。下载前后远端原件 SHA-256 分别为：

```text
ce4b6ba06302ca6522ab418d3046a9cc4a3873d35fc13e10829aaba65dbe0243  val_mujoco_g1_armhack_stand.sh
27e0a3a4b340ae9ffd35bbe7d951f016ba25d8257b0a7298805d12977a1519ce  deploy_real_g1_armhack_stand.sh
```

可重复执行的同步命令如下；第一条脚本随后会因加入随机轨迹模式而产生新的本机 SHA，第二条保持与 HEC 原件完全一致：

```bash
cd /home/user/Workspace/Humanoid/Locomotion/G1-Locomotion

scp -T -i /home/user/.ssh/hec5090_codex_monitor_ed25519 \
  HEC-5090:/home/hecggdz/G1-ARM-HACK-WALK/LocomotionHEC/legged_lab/scripts/val_mujoco_g1_armhack_stand.sh \
  legged_lab/scripts/val_mujoco_g1_armhack_stand.sh

scp -T -i /home/user/.ssh/hec5090_codex_monitor_ed25519 \
  HEC-5090:/home/hecggdz/G1-ARM-HACK-WALK/LocomotionHEC/scripts/deploy_real_g1_armhack_stand.sh \
  scripts/deploy_real_g1_armhack_stand.sh

chmod +x legged_lab/scripts/val_mujoco_g1_armhack_stand.sh \
  scripts/deploy_real_g1_armhack_stand.sh
bash -n legged_lab/scripts/val_mujoco_g1_armhack_stand.sh
bash -n scripts/deploy_real_g1_armhack_stand.sh
```

当前第三阶段模型的 MuJoCo 无头验证命令为：

```bash
cd /home/user/Workspace/Humanoid/Locomotion/G1-Locomotion/legged_lab

CHECKPOINT="$PWD/../checkpoint/stand/model_2999.pt" \
MODE=randomized_trajectory ITEM=1 PAYLOAD_KG=0 \
USE_GLFW=False REAL_TIME=False \
bash scripts/val_mujoco_g1_armhack_stand.sh
```

真机入口只执行了离线 `DRY_RUN`，没有初始化 DDS，也没有向机器人发命令：

```bash
cd /home/user/Workspace/Humanoid/Locomotion/G1-Locomotion

DRY_RUN=True NET=enp11s0 \
POLICY_PATH="$PWD/checkpoint/stand/stand_robust_model_2999.onnx" \
EXPECTED_POLICY_SHA256=354bf4b35572cf6d91d44d448cde36b7bb748cffe40f1e220183bf21e5553fbf \
UNITREE_PYTHON=/home/user/anaconda3/envs/gmr/bin/python \
bash scripts/deploy_real_g1_armhack_stand.sh
```

验证输出为 `[SELF-TEST PASS]`：actor 输入/输出为 `96→29`，全零和 16 组确定性随机输入均为有限值；6 个双臂预设、25.5 s 初始化 CSV、SPACE 锁和 7.5 s minimum-jerk 循环通过，最坏相邻 SPACE 预设峰值速度约 `0.248 rad/s`。这只证明模型、预设、组合动作和安全入口的离线契约可运行，不代表真机已经部署成功。

#### 1.1.9.2 可复现的更多随机双臂轨迹

新增 `MODE=generated_random_trajectory`。它从训练使用的 512 姿态库中按 seed **无放回**抽取任意数量的 14-DoF 双臂姿态，相邻姿态用五次 minimum-jerk 插值连接。`RANDOM_WAYPOINTS=N` 会产生 `N-1` 条随机过渡轨迹；同一组参数和 seed 会生成字节完全一致的 CSV，改变 `RANDOM_SEED` 即可得到另一组轨迹。生成器只允许 manifest 中的 14 个肩、肘、腕关节，不读取或写入根节点、腰、髋、膝和踝。

每段过渡的实际时长为 `max(RANDOM_TRANSITION_S, 1.875 × max(|Δq|/v_limit))`，再向上取整到 50 Hz 控制帧，所以 `RANDOM_TRANSITION_S` 是名义最短时长，不会为了凑固定时长突破训练姿态库给出的关节速度上限。生成器还会检查：姿态库 SHA 与 manifest 一致、数值有限、时间单调、CSV 恰好 15 列、轨迹范围不越界、离散峰值速度不超限。

轨迹和生成元数据保存到清晰的相对路径：

```text
Reference Data/ArmHack/StandPerturb/TestData/ArmOnly/trajectories/generated/
  generated_random_arm_trajectory_seed<seed>_wp<数量>_hold<时长>s_transition<时长>s_50hz.csv
  generated_random_arm_trajectory_seed<seed>_wp<数量>_hold<时长>s_transition<时长>s_50hz.json
```
...
GUI 可视化使用完全相同的数据生成参数，只切换 GLFW 和实时播放：
...
512 姿态库的逐关节范围就是随机轨迹的硬范围；minimum-jerk 是端点之间的凸插值，不会越出该范围：

| 双臂关节 | 最小值 rad | 最大值 rad | 插值速度上限 rad/s |
|---|---:|---:|---:|
| left_shoulder_pitch_joint | -0.811248 | 0.279171 | 0.387553 |
| left_shoulder_roll_joint | 0.437796 | 1.064786 | 0.200000 |
| left_shoulder_yaw_joint | -0.654099 | 0.123821 | 0.211684 |
| left_elbow_joint | -0.938281 | 0.441631 | 0.498593 |
| left_wrist_roll_joint | 0.248385 | 0.867490 | 0.200000 |
| left_wrist_pitch_joint | -0.252484 | 0.747216 | 0.361817 |
| left_wrist_yaw_joint | -0.969488 | 0.047236 | 0.477206 |
| right_shoulder_pitch_joint | -0.899764 | 0.280394 | 0.422175 |
| right_shoulder_roll_joint | -0.990113 | -0.430222 | 0.216559 |
| right_shoulder_yaw_joint | -0.123126 | 0.691010 | 0.213857 |
| right_elbow_joint | -0.594729 | 0.483144 | 0.502645 |
| right_wrist_roll_joint | -0.867418 | -0.249536 | 0.200000 |
| right_wrist_pitch_joint | -0.252412 | 0.612142 | 0.368062 |
| right_wrist_yaw_joint | -0.046738 | 1.034754 | 0.480617 |

2026-07-20 已用当前第三阶段模型 `model_2999.pt`（SHA 前缀 `146aca1f547c`）完整实跑两组新轨迹：

| seed / 姿态点 / 负载 | 时长 / 样本 | 完整播放 | health / 摔倒 | 最低 root 高度 | max abs roll/pitch | torso 水平位移 RMS/最大值 | 双臂跟踪 MAE/RMS/最大值 |
|---|---:|---|---|---:|---:|---:|---:|
| 20260720 / 8 / 0 kg | 20.56 s / 1029 | 是 | `True` / 无 | 0.788 m | 0.008 / 0.044 rad | 0.01227 / 0.01634 m | 0.05441 / 0.06546 / 0.17356 rad |
| 20260721 / 10 / 每侧 1 kg | 24.10 s / 1206 | 是 | `True` / 无 | 0.788 m | 0.021 / 0.118 rad | 0.01167 / 0.01700 m | 0.07611 / 0.09108 / 0.24400 rad |

两组都成功生成、完整播放并保持 `healthy=True`；1 kg 组的最大 pitch 与双臂跟踪误差更大。报告分别保存在当前 checkpoint 同级的 `Test Reports/StandArmOnlyMuJoCo/`，文件名含模型短 SHA、seed、姿态点数、过渡参数和 payload，不会与历史同名 `model_2999.pt` 报告混淆。6D 曲线会读取同名 JSON，把每个 `W01…WN` 姿态保持段和中间过渡段画在时间轴上，并在绘图前校验 JSON 中记录的 CSV SHA。这里仍只是 MuJoCo sim2sim 验证，不等价于真机稳定性结论。
...
当前已新增 Stand 专用真机入口 `../scripts/deploy_real_g1_armhack_stand.sh` 和控制器 `../unitree_sim2sim2real/deploy/deploy_real/deploy_real_g1_armhack_stand.py`。它复用通用 96 维 observation/29 维 actor，但在每帧把 14 维双臂位置覆盖成预设 minimum-jerk 轨迹，并把实际发送的完整组合 action 回写到下一帧 `obs[67:96]`；Stand 速度命令被强制为 `[0,0,0]`。当前输出已与 HEC-5090 通用部署链对齐，不再做 `0.05 rad` 安全余量裁剪或部署层逐帧目标限速。
...
2026-07-21 起，MuJoCo 与真机共用同一 schema v2 双臂状态机和同一份 25.5 s 初始化 CSV。启动时处于自然下垂的阻尼/待机模式；真机此时尚未初始化 DDS、也不发送 LowCmd。操作员按一次 `ENTER` 后才进行低层接管并启动 50 Hz actor；从 actor 第一帧开始，腰腿 15 维始终由模型推理，双臂 14 维依次执行 `自然下垂→平直默认 P0→向前伸直 F→收回平直默认 P0`。三段都是 7.5 s minimum-jerk，并在每个到达点保持 1 s，总长 25.5 s。初始化结束前 `SPACE` 会被拒绝；完成后 `SPACE` 按 `P0→P1→P2→P3→P0` 循环切换，每次 7.5 s。`q`、`Ctrl-C` 或遥控器 `Select` 退出并发送阻尼。

真机最终目标输出已与 HEC-5090 的 `scripts/deploy_real_g1_amp_onnx.sh` 调用链对齐：使用 `default_angles + action * action_scale` 直通语义，不做 `0.05 rad` 安全余量裁剪，也不做 `4.0 rad/s` 逐帧目标变化限速。双臂 7.5 s minimum-jerk 轨迹仍保留；low-state 超时、NaN/Inf、实测关节越硬件范围、躯干倾角和 Enter 前姿态检查也仍保留。

当前真机 launcher 默认锁定最新 HEC-5090 鲁棒模型导出件：
...
窗口启动后先看到自然下垂待机，按一次 `ENTER` 启动模型和自动初始化；终端打印 `INIT COMPLETE` 后再按 `SPACE` 切换姿态，按 `Q` 结束。下面的无头命令会自动模拟一次 Enter 和两次 Space，适合回归测试，不需要人工按键：
...
当前实跑结果为 `policy_activation_time_s=0.5`、`startup_complete=True`、`space_switch_count=2`、`complete_csv_playback=True`、`healthy=True`，全程未摔倒。报告位于 `../checkpoint/stand/Test Reports/StandArmOnlyMuJoCo/model_2999_146aca1f547c__mujoco__interactive__payload_0kg.md`。预设数据位于 `Reference Data/ArmHack/StandPerturb/RealDeployment/stand_arm_presets.json`，初始化 CSV 位于 `Reference Data/ArmHack/StandPerturb/TestData/ArmOnly/special/arms_down_flat_forward_return_flat_25p5s_50hz.csv`；两者都只包含双臂 14DoF。完整安全顺序见 `docs/armhack_stand_real_deployment.md`。真机目前只完成离线 `DRY_RUN`，没有初始化 DDS，也没有向机器人发命令，不代表已完成吊架验收。

### 1.1.11 从自然下垂初始化到平直 P0 的第四阶段续训

第三阶段虽然覆盖了 512 个姿态、随机轨迹、末端负载、torso 外力和关节参数随机化，但 reset 姿态由随机姿态库给出，不能保证策略在真机最初的双臂自然下垂状态下站稳，也没有把“自然下垂后以不同速度同时抬臂到平直默认位置”作为固定训练分布。第四阶段只修复这个 Stand 分布缺口，不修改 Walk 环境、Walk 奖励或 Walk 脚本。

新增任务和入口为：

```text
task: LeggedLab-Isaac-AMP-G1-StandDownToDefault-v0
config: source/legged_lab/legged_lab/tasks/locomotion/amp/config/g1_perturb/g1_stand_down_to_default_env_cfg.py
script: scripts/train_g1_armhack_stand_down_to_default.sh
base: ../checkpoint/stand/model_2999.pt
base SHA-256: 146aca1f547ce073756c942508e8ea43c8cea91b27eee3b8347dd4131c87bc5f
```

训练中的起点和终点不另复制一份数字，而是通过 `reference_data.load_stand_arm_pose()` 从 MuJoCo/真机共用文件 `Reference Data/ArmHack/StandPerturb/RealDeployment/stand_arm_presets.json` 读取：

- 起点 `AD_natural_down`：直立机器人双臂自然下垂；每次 reset 在第一帧 policy observation 前，把仿真的 14 个手臂关节位置直接写成 AD、速度写成零，因此没有从旧平直姿态跳到 AD 的初始化冲击；
- 终点 `P0_symmetric_reference`：当前部署状态机使用的双臂平直默认位置；
- 每个环境、每个 episode 独立采样 `2–5 s` 的 AD 保持时间和 `3–9 s` 的抬臂时长；抬臂用五次 minimum-jerk 插值；
- 左右臂的 14 个关节共用同一个插值 phase，所以两臂同时开始、同时到达；不同并行环境的 duration 不同，因此训练会同时看到不同抬臂速度；
- 到达 P0 后保持 P0，episode 继续沿用 20 s；
- 继承第三阶段的左右腕端 `0–1 kg` 负载、torso 周期外力、actuator gain 与关节 friction/armature 随机化，以及 `-500` 非 timeout 摔倒惩罚。

课程按 runner 每个 PPO iteration 收集 24 个控制步计算：前 `12,000` 步约 500 iteration 的 motion scale 为 0，机器人只在 AD 下静止站立；随后 `12,000` 步约 500 iteration 从 0 线性升到 1；剩余约 1000 iteration 使用完整的 `2–5 s` 保持和 `3–9 s` 抬臂分布。policy observation 仍为 96 维，只能看到当前机器人状态、零速度指令和上一动作；未来 P0、随机等待时长、抬臂时长、当前 phase 及未来手臂动作都没有加入 observation。

新终端启动正式 `4096 env × 2000 iteration` 续训：
...
bash scripts/train_g1_armhack_stand_down_to_default.sh
```

可通过环境变量调整规模和速度范围，例如 `NUM_ENVS=2048 MAX_ITERATIONS=2000 START_DELAY_MIN_S=2 START_DELAY_MAX_S=5 LIFT_DURATION_MIN_S=3 LIFT_DURATION_MAX_S=9 bash scripts/train_g1_armhack_stand_down_to_default.sh`。脚本会同时校验基线 checkpoint 的大小和 SHA，采用 policy-only 加载、重置 iteration 计数并用 `0.001` 冻结基线 KL 约束更新。正式 checkpoint 仍保存到：

```text
ArmHack Checkpoints/StandPerturb/<时间戳>_<RUN_NAME>/model_*.pt
```

本机 2026-07-21 已执行以下真实 Isaac smoke。这里故意取消等待、把抬臂缩短到 `0.3–0.5 s`，目的是在单个 24-step rollout 内确认每个环境能够走完整个 `AD→P0` 路径；这不是正式训练参数：
...
RUN_NAME=smoke_down_to_default_mapfix_20260721 \
QUIET_TERMINAL=False HEADLESS=True \
STATIC_STEPS=0 RAMP_STEPS=0 \
START_DELAY_MIN_S=0 START_DELAY_MAX_S=0 \
LIFT_DURATION_MIN_S=0.3 LIFT_DURATION_MAX_S=0.5 \
bash scripts/train_g1_armhack_stand_down_to_default.sh
```

smoke 结果为退出码 0，成功加载 HEC-5090 保存的基线策略、创建 `96→29` actor、收集 `8×24=192` step、完成一次 PPO 更新并保存 `model_0.pt`；日志中 `ArmHack/down_to_default_motion_scale=1.0`、平均 duration `0.3630 s`、`ArmHack/down_to_default_progress_mean=1.0`，说明 8 个环境都已到达 P0，短 rollout 中 `base_height=0`、`bad_orientation=0`。运行与 checkpoint 分别位于：

```text
logs/rsl_rl/g1_stand_perturb/2026-07-21_13-39-12_smoke_down_to_default_mapfix_20260721/
ArmHack Checkpoints/StandPerturb/2026-07-21_13-39-12_smoke_down_to_default_mapfix_20260721/model_0.pt
```

该 `model_0.pt` 只用于证明任务、数据、跨 GPU checkpoint 加载、仿真 rollout、PPO 和保存链无报错，不能部署。第三阶段 checkpoint 来自服务器 `cuda:1`，因此训练入口的 runner 已使用 `map_location=agent_cfg.device` 加载，使单卡本机 `cuda:0` 能安全恢复同一策略。
...
| Stand 自然下垂→平直 P0 续训配置 | `source/legged_lab/legged_lab/tasks/locomotion/amp/config/g1_perturb/g1_stand_down_to_default_env_cfg.py` |
...
| Stand 从鲁棒 model2999 自然下垂→平直 P0 续训 | `scripts/train_g1_armhack_stand_down_to_default.sh` |
| Stand 固定姿态/轨迹/负载回放 | `scripts/vis_g1_armhack_stand_eval.sh` |
| Stand MuJoCo 导出、测试与 GUI 入口 | `scripts/val_mujoco_g1_armhack_stand.sh` |
| Stand MuJoCo 可复现随机双臂轨迹生成器 | `scripts/tools/generate_armhack_stand_random_trajectory.py` |
...
train_g1_armhack_stand_down_to_default.sh
  -> 校验第三阶段鲁棒 model_2999 SHA-256/大小并建立只读恢复软链接
  -> train_g1_amp.sh -> Gym StandDownToDefault task
  -> reset 写入共享 AD 自然下垂姿态，按环境采样等待时间和抬臂时长
  -> 双臂共用 minimum-jerk phase 同时到达共享 P0
  -> AMPRunner / PPOAMP.learn()
...
| 当前 schema v5 `1.0x` 确定性数据集 | 512 姿态库、默认 37 个可回放 CSV、3 个双臂专项 CSV、59 阶段默认时间线均校验通过；默认全量和专项均可生成独立逐关节与 6D 报告 | 多 seed、多 episode 的训练分布统计，以及与旧模型在完全相同 schema v5 条件下的性能对照 |
...
训练姿态库输出到 `Reference Data/ArmHack/StandPerturb/RandomizedTraining/`，测试集输出到 `Reference Data/ArmHack/StandPerturb/TestData/ArmOnly/`。`manifest.json` 当前为 schema v5，显式记录 `data_scope=arm_only_14_dof`、`contains_full_body_state=false`、训练采样边界、姿态库 SHA-256、`files.all.detailed_timeline` 和三个 `special_tests`；默认时间线共 59 段，供 `MODE=all` 的 6D 测试图阶段色带使用。默认 37 个 CSV 加上 3 个专项 CSV，共 40 个可回放 CSV，表头都严格是 `time_s + 14 个双臂关节`，不含根节点、腰、髋、膝或踝。运行时 seed 生成的 `trajectories/generated/` 文件是额外测试产物，不计入 schema v5 固定文件数。选样和生成规则如下：
...
ROBUST_CKPT="$PWD/../checkpoint/stand/model_2999.pt"
test -f "$ROBUST_CKPT" || { echo "checkpoint 不存在: $ROBUST_CKPT"; exit 1; }
sha256sum "$ROBUST_CKPT"
...
第三阶段正式训练已完成，但最终判断仍必须把新模型与 SHA 前缀 `877e929d516c` 的输入模型放在同一
schema、同一 payload 和同一 step 数下比较；重点看 termination/reset、torso 世界系 6D、pitch
RMS/最大值、水平漂移和逐关节波动。当前新增随机轨迹的 MuJoCo 结果见第 1.1.9 节；这些 sim2sim
通过结果仍不能代替真机吊架验收，也不能单独证明真机稳定性已经改善。
