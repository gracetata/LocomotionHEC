...
TASK=${TASK:-"LeggedLab-Isaac-AMP-G1-ExtremeStandRecovery-v0"}
DISTURBANCE_MODE=${DISTURBANCE_MODE:-legacy}
...
[[ "${DISTURBANCE_MODE}" == "legacy" || "${DISTURBANCE_MODE}" == "single" ]] || \
    die "DISTURBANCE_MODE must be legacy or single, got ${DISTURBANCE_MODE}."
...
DISTURBANCE_ARGS=()
if [[ "${DISTURBANCE_MODE}" == "legacy" ]]; then
    DISTURBANCE_ARGS=(
