...
elif args_cli.robot == "t1":
    from legged_lab.assets.roboparty import T1_29DOF_CFG as ROBOT_CFG
else:
...
if __name__ == "__main__":
    ...
    for index, jnt in enumerate(lab_joint_names):
        print(f"{index:2d}: {jnt}")

    body_names = robot.body_names
    body_masses = robot.data.default_mass[0]
    for index, (name, mass) in enumerate(zip(body_names, body_masses)):
        print(f"Body {index:2d}: {name}, Mass: {mass} kg")