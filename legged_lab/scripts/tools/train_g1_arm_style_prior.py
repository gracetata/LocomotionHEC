#!/usr/bin/env python3
"""Train a lightweight G1 arm-style prior from washed CMU walking demos.

The exported checkpoint is consumed by ``mdp.arm_style_prior_exp``. It predicts
the 14 G1 arm joint positions from root velocity plus non-arm joint
positions/velocities, using the same metadata schema as the existing T1 prior.
"""
...
INPUT_JOINT_NAMES = [name for name in G1_LOCOMOTION_JOINT_NAMES if name not in ARM_JOINT_NAMES]
...
def root_quat_from_motion(motion: dict, root_rot_order: str, motion_path: Path) -> np.ndarray:
    """Load root quaternion in wxyz order."""

    root_quat = np.asarray(motion["root_rot"], dtype=np.float32)
    order = root_rot_order.lower()
    if order == "xyzw":
        return np.ascontiguousarray(root_quat[:, [3, 0, 1, 2]])
    if order == "wxyz":
        return np.ascontiguousarray(root_quat)
    raise ValueError(f"Unsupported root_rot_order={root_rot_order!r} for {motion_path}")


def aligned_dof_pos(motion: dict, motion_path: Path) -> np.ndarray:
    """Return DoF positions aligned to G1_LOCOMOTION_JOINT_NAMES."""

    dof_pos = np.asarray(motion["dof_pos"], dtype=np.float32)
    source_names = motion.get("dof_names")
    if source_names is None:
        raise KeyError(f"{motion_path} does not contain dof_names; cannot safely align G1 joints.")
    source_names = list(source_names)
    if dof_pos.shape[1] != len(source_names):
        raise ValueError(
            f"{motion_path} dof_pos width and dof_names length disagree: "
            f"{dof_pos.shape[1]} != {len(source_names)}"
        )
    index_by_name = {name: index for index, name in enumerate(source_names)}
    missing = [name for name in G1_LOCOMOTION_JOINT_NAMES if name not in index_by_name]
    if missing:
        raise ValueError(f"{motion_path} missing G1 joints: {missing}")
    return np.ascontiguousarray(dof_pos[:, [index_by_name[name] for name in G1_LOCOMOTION_JOINT_NAMES]])


def load_dataset(motion_dir: Path, root_rot_order: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    name_to_index = {name: index for index, name in enumerate(G1_LOCOMOTION_JOINT_NAMES)}
    ...
    for motion_path in motion_paths:
        ...
        root_quat = root_quat_from_motion(motion, root_rot_order, motion_path)
        dof_pos = aligned_dof_pos(motion, motion_path)
...
def train_model(args: argparse.Namespace) -> dict:
    ...
    features, targets = load_dataset(Path(args.motion_dir), args.root_rot_order)
    ...
    return {
        ...
        "joint_order": G1_LOCOMOTION_JOINT_NAMES,
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Train a G1 arm-style prior from Lab-format motion data.")
    parser.add_argument("--motion_dir", type=str, required=True)
    parser.add_argument("--output", type=str, default="logs/arm_style_prior/g1_cmu_walk_washed_arm_prior.pt")
    ...
    print(f"[INFO] Saved G1 arm style prior: {output_path}")
