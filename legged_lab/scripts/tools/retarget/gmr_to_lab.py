...
if not hasattr(np, "_core"):
    ...
    _shim.__dict__.update(_nc.__dict__)
...
from t1_lab_joint_offsets import compensate_t1_lab_baked_zero_offsets
...
def extract_gmr_data(
    ...
    for lab_dof in lab_dof_names:
        if lab_dof in gmr_dof_names:
            ...
            gmr_to_lab_indices.append(None)
            missing_lab_dofs.append(lab_dof)

    if missing_lab_dofs:
        print(f"[INFO] Filling missing DOFs with zeros: {missing_lab_dofs}")

    dof_pos_lab = np.zeros((dof_pos.shape[0], len(lab_dof_names)), dtype=dof_pos.dtype)
    for lab_idx, gmr_idx in enumerate(gmr_to_lab_indices):
        if gmr_idx is not None:
            dof_pos_lab[:, lab_idx] = dof_pos[:, gmr_idx]
    dof_pos_lab = compensate_t1_lab_baked_zero_offsets(dof_pos_lab, lab_dof_names)
