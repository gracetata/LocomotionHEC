...
def _transform_policy_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    # Current AMP policy obs layout: [base_ang_vel(3), projected_gravity(3), velocity_commands(3), joint_pos(29), joint_vel(29), actions(29)]
    if obs.shape[-1] == 96:
        ...
        obs[:, 9 : 9 + joint_num] = _switch_g1_29dof_joints_left_right(obs[:, 9 : 9 + joint_num])
        obs[:, 38 : 38 + joint_num] = _switch_g1_29dof_joints_left_right(obs[:, 38 : 38 + joint_num])
        obs[:, 67 : 67 + joint_num] = _switch_g1_29dof_joints_left_right(obs[:, 67 : 67 + joint_num])
        return obs

    # Legacy fallback for older policy layouts with history and key-body terms.
    key_body_num = 6
    HISTORY_LEN = 5
    ANG_VEL_DIM = 3
    ROT_TAN_NORM = 6
