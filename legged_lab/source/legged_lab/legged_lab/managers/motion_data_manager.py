...
# pkl files saved with numpy >= 2.0 reference 'numpy._core' internally.
# numpy < 2.0 (e.g. 1.26.x) uses 'numpy.core' instead.  Provide an alias so
# joblib.load() can deserialize those files in older environments.
# ---------------------------------------------------------------------------
if not hasattr(np, "_core"):
    import numpy.core as _numpy_core
    import types as _types
    _shim = _types.ModuleType("numpy._core")
    _shim.__dict__.update(_numpy_core.__dict__)
    sys.modules.setdefault("numpy._core", _shim)
    # also register common sub-modules that pickle may reference
    for _sub in ("multiarray", "umath", "numeric", "numerictypes", "fromnumeric"):
        _full = f"numpy._core.{_sub}"
        if _full not in sys.modules:
            _src_name = f"numpy.core.{_sub}"
            try:
                import importlib as _importlib
                _src_mod = _importlib.import_module(_src_name)
                sys.modules[_full] = _src_mod
            except ImportError:
                pass
...
class MotionDataTerm(ManagerTermBase):
    ...
    def __init__(self, cfg: MotionDataTermCfg, env: ManagerBasedEnv):
        ...
        self._warned_declared_dof_names_without_target = False
            
        self._load_motion_data()

    def _root_quat_from_raw(self, motion_raw_data: dict, motion_file: str) -> torch.Tensor:
        """Load root quaternions into Legged Lab's internal ``wxyz`` order."""

        root_rot_order = getattr(self.cfg, "root_rot_order", "wxyz").lower()
        if root_rot_order not in ("wxyz", "xyzw"):
            raise ValueError(
                f"[MotionLoader] Unsupported root_rot_order={root_rot_order!r} for {motion_file}. "
                "Expected 'wxyz' or 'xyzw'."
            )

        root_rot = np.asarray(motion_raw_data["root_rot"], dtype=np.float32)
        if root_rot_order == "xyzw":
            root_rot = root_rot[:, [3, 0, 1, 2]]
            print(f"[MotionLoader] Converted root_rot xyzw -> wxyz for {motion_file}.")
        return torch.from_numpy(np.ascontiguousarray(root_rot)).to(self.device).float()

    def _dof_pos_from_raw(self, motion_raw_data: dict, motion_file: str) -> torch.Tensor:
        """Load DoF positions, optionally aligning source names to a target order.

        Reference motion must not be consumed by raw index when a converter
        declares a different ``dof_names`` order. The CMU/GMR G1 pickles store
        MuJoCo order while the G1 AMP policy uses an interleaved Legged Lab
        order; raw-index consumption makes the reference motion visibly malformed.
        """

        dof_pos = np.asarray(motion_raw_data["dof_pos"], dtype=np.float32)
        source_dof_names = motion_raw_data.get("dof_names")
        target_dof_names = getattr(self.cfg, "target_dof_names", None)
        strict_dof_names = bool(getattr(self.cfg, "strict_dof_names", False))

        if target_dof_names is None:
            if source_dof_names is not None and not self._warned_declared_dof_names_without_target:
                print(
                    "[MotionLoader] Warning: motion files declare dof_names, but no target_dof_names "
                    "was configured. Reference DoFs will be consumed by raw index."
                )
                self._warned_declared_dof_names_without_target = True
            return torch.from_numpy(np.ascontiguousarray(dof_pos)).to(self.device).float()

        if source_dof_names is None:
            message = (
                f"[MotionLoader] target_dof_names configured for {motion_file}, but the motion file "
                "does not contain dof_names."
            )
            if strict_dof_names:
                raise ValueError(message)
            print(f"{message} Falling back to raw index order.")
            return torch.from_numpy(np.ascontiguousarray(dof_pos)).to(self.device).float()

        source_dof_names = list(source_dof_names)
        if dof_pos.shape[1] != len(source_dof_names):
            raise ValueError(
                f"[MotionLoader] dof_pos width and dof_names length disagree for {motion_file}: "
                f"dof_pos={dof_pos.shape[1]}, dof_names={len(source_dof_names)}."
            )

        source_index_by_name = {name: index for index, name in enumerate(source_dof_names)}
        missing_names = [name for name in target_dof_names if name not in source_index_by_name]
        if missing_names:
            raise ValueError(f"[MotionLoader] Missing target DoFs in {motion_file}: {missing_names}")

        permutation = [source_index_by_name[name] for name in target_dof_names]
        if permutation != list(range(len(permutation))):
            print(f"[MotionLoader] Reordered dof_pos by dof_names for {motion_file}.")
        dof_pos = dof_pos[:, permutation]
        return torch.from_numpy(np.ascontiguousarray(dof_pos)).to(self.device).float()
    ...
    def _load_motion_data(self):
        ...
        for motion_name, motion_weight in self.motion_weights_dict.items():
            ...
            loop_mode = motion_raw_data.get("loop_mode", LoopMode.CLAMP.value)
            ...
            # root rotation from world frame to body frame, stored internally as (w, x, y, z)
            root_quat = self._root_quat_from_raw(motion_raw_data, motion_file)
            ...
            # dof position, shape (num_frames, num_dofs), optionally reordered by name
            dof_pos = self._dof_pos_from_raw(motion_raw_data, motion_file)
            ...
            if "key_body_pos" in motion_raw_data:
                ...
                key_body_pos_b = torch.from_numpy(motion_raw_data["local_body_pos"]).to(self.device).float()
                root_quat_w = root_quat.unsqueeze(1).expand(-1, key_body_pos_b.shape[1], -1)
                key_body_pos_w = root_pos_w.unsqueeze(1) + math_utils.quat_apply(root_quat_w, key_body_pos_b)
            else:
                raise KeyError(f"Motion data file {motion_file} must contain 'key_body_pos' or 'local_body_pos'.")
