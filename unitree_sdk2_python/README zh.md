...
终端中执行：
```bash
python3 ./example/g1/high_level/read_highstate.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。
### 高层控制
终端中执行：
```bash
python3 ./example/g1/high_level/sportmode_test.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。
...
终端中执行：
```bash
python3 ./example/low_level/lowlevel_control.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。程序会输出右前腿hip关节的状态、IMU和电池电压信息。
...
python3 ./example/low_level/lowlevel_control.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。左后腿 hip 关节会保持在0角度 (安全起见，这里设置 kp=10, kd=1)，左后腿 calf 关节将持续输出 1Nm 的转矩。

## 遥控器状态获取
终端中执行：
```bash
python3 ./example/wireless_controller/wireless_controller.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。
...
```bash
python3 ./example/front_camera/camera_opencv.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。

## 避障开关
```bash
python3 ./example/obstacles_avoid_switch/obstacles_avoid_switch.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。机器人将循环开启和关闭避障功能。关于避障服务，详细见 https://support.unitree.com/home/zh/developer/ObstaclesAvoidClient

## 灯光音量控制
```bash
python3 ./example/vui_client/vui_client_example.py enp11s0
```
其中 `enp11s0` 为机器人所连接的网卡名称，请根据实际情况修改。机器人将循环调节音量和灯光亮度。该接口详细见 https://support.unitree.com/home/zh/developer/VuiClient
