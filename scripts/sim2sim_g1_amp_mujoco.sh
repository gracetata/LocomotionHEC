...
#   POLICY_PATH         : TorchScript policy.pt or ONNX policy.onnx exported by scripts/export_g1_amp_policy.sh.
#   POLICY_RUNTIME      : auto, torchscript, or onnx (default: auto).
...
#   JOINT_RANDOM_*      : Optional initial joint qpos/qvel randomization for stability sweeps.
# ============================================================
: <<'BLOCK'
# [历史记录与快捷执行指令存放区]
...
POLICY_RUNTIME=${POLICY_RUNTIME:-auto}
...
JOINT_POS_NOISE_RAD=${JOINT_POS_NOISE_RAD:-0.0}
JOINT_VEL_NOISE_RAD_PER_S=${JOINT_VEL_NOISE_RAD_PER_S:-0.0}
...
NON_ARM_JOINT_TARGET_NOISE_RAD=${NON_ARM_JOINT_TARGET_NOISE_RAD:-0.0}
...
INITIAL_ANKLE_DISTANCE_M=${INITIAL_ANKLE_DISTANCE_M:-0.30}
INTERACTIVE_STANCE_RESET=${INTERACTIVE_STANCE_RESET:-False}
INTERACTIVE_STANCE_DISTANCE_RANGE_M=${INTERACTIVE_STANCE_DISTANCE_RANGE_M:-'[0.08,0.48]'}
...
if ! "${UNITREE_PYTHON}" - "${POLICY_PATH}" "${POLICY_RUNTIME}" <<'PY' >/dev/null 2>&1
import sys

policy_path = sys.argv[1]
runtime = sys.argv[2].strip().lower()
if runtime == "auto":
    runtime = "onnx" if policy_path.endswith(".onnx") else "torchscript"
if runtime in {"torch", "jit"}:
    runtime = "torchscript"
...
export G1_AMP_POLICY_RUNTIME="${POLICY_RUNTIME}"
...
export G1_AMP_JOINT_RANDOM_ENABLE="${JOINT_RANDOM_ENABLE}"
export G1_AMP_JOINT_RANDOM_SEED="${JOINT_RANDOM_SEED}"
export G1_AMP_JOINT_POS_NOISE_RAD="${JOINT_POS_NOISE_RAD}"
export G1_AMP_JOINT_VEL_NOISE_RAD_PER_S="${JOINT_VEL_NOISE_RAD_PER_S}"
export G1_AMP_NON_ARM_JOINT_TARGET_NOISE_ENABLE="${NON_ARM_JOINT_TARGET_NOISE_ENABLE}"
export G1_AMP_NON_ARM_JOINT_TARGET_NOISE_SEED="${NON_ARM_JOINT_TARGET_NOISE_SEED}"
export G1_AMP_NON_ARM_JOINT_TARGET_NOISE_RAD="${NON_ARM_JOINT_TARGET_NOISE_RAD}"
export G1_AMP_FOOT_RECOVERY_ENABLE="${FOOT_RECOVERY_ENABLE}"
export G1_AMP_ORDERED_STEP_OBSERVATION_ENABLE="${ORDERED_STEP_OBSERVATION_ENABLE}"
export G1_AMP_ORDERED_STEP_MIRROR_POLICY_ENABLE="${ORDERED_STEP_MIRROR_POLICY_ENABLE}"
export G1_AMP_ORDERED_STEP_HOLD_LAST_ACTION_ENABLE="${ORDERED_STEP_HOLD_LAST_ACTION_ENABLE}"
export G1_AMP_ORDERED_STEP_HOLD_POLICY_PATH="${ORDERED_STEP_HOLD_POLICY_PATH}"
export G1_AMP_ORDERED_STEP_HOLD_BLEND_DURATION_S="${ORDERED_STEP_HOLD_BLEND_DURATION_S}"
export G1_AMP_ORDERED_STEP_TRANSITION_TOLERANCE_M="${ORDERED_STEP_TRANSITION_TOLERANCE_M}"
export G1_AMP_ORDERED_STEP_MIN_CLEARANCE_M="${ORDERED_STEP_MIN_CLEARANCE_M}"
export G1_AMP_ORDERED_STEP_MIN_DURATION_S="${ORDERED_STEP_MIN_DURATION_S}"
export G1_AMP_ORDERED_STEP_ACTION_SMOOTHING_ALPHA="${ORDERED_STEP_ACTION_SMOOTHING_ALPHA}"
export G1_AMP_INITIAL_ANKLE_DISTANCE_M="${INITIAL_ANKLE_DISTANCE_M}"
export G1_AMP_INTERACTIVE_STANCE_RESET="${INTERACTIVE_STANCE_RESET}"
export G1_AMP_INTERACTIVE_STANCE_DISTANCE_RANGE_M="${INTERACTIVE_STANCE_DISTANCE_RANGE_M}"
export G1_AMP_INTERACTIVE_STANCE_SEED="${INTERACTIVE_STANCE_SEED}"
export G1_AMP_TARGET_ANKLE_DISTANCE_M="${TARGET_ANKLE_DISTANCE_M}"
export G1_AMP_ANKLE_DISTANCE_TOLERANCE_M="${ANKLE_DISTANCE_TOLERANCE_M}"
export G1_AMP_ANKLE_CONVERGENCE_HOLD_S="${ANKLE_CONVERGENCE_HOLD_S}"
export G1_AMP_RECOVERY_SETTLE_TIME_S="${RECOVERY_SETTLE_TIME_S}"
export G1_AMP_MUJOCO_PUSH_ENABLE="${MUJOCO_PUSH_ENABLE}"
export G1_AMP_MUJOCO_PUSH_SEED="${MUJOCO_PUSH_SEED}"
export G1_AMP_MUJOCO_PUSH_FIRST_TIME_S="${MUJOCO_PUSH_FIRST_TIME_S}"
export G1_AMP_MUJOCO_PUSH_INTERVAL_RANGE_S="${MUJOCO_PUSH_INTERVAL_RANGE_S}"
export G1_AMP_MUJOCO_PUSH_DURATION_S="${MUJOCO_PUSH_DURATION_S}"
export G1_AMP_MUJOCO_PUSH_FORCE_RANGE_N="${MUJOCO_PUSH_FORCE_RANGE_N}"
export G1_AMP_MUJOCO_PUSH_YAW_TORQUE_RANGE_NM="${MUJOCO_PUSH_YAW_TORQUE_RANGE_NM}"
...
echo "Space Reset : ${INTERACTIVE_STANCE_RESET} range=${INTERACTIVE_STANCE_DISTANCE_RANGE_M}m"
...
echo "NonArm Noise: ${NON_ARM_JOINT_TARGET_NOISE_ENABLE} seed=${NON_ARM_JOINT_TARGET_NOISE_SEED} target=+/-${NON_ARM_JOINT_TARGET_NOISE_RAD}rad"
echo "====================================="
...
if [[ "${PYTHON_STATUS}" -ne 0 ]]; then
    ...
    if [[ -f "${METRICS_PATH}" ]]; then
        METRICS_MTIME=$(stat -c %Y "${METRICS_PATH}" 2>/dev/null || echo 0)
    fi
    if [[ ( "${PYTHON_STATUS}" -eq 134 || "${PYTHON_STATUS}" -eq 139 ) && "${USE_GLFW,,}" == "true" && "${METRICS_MTIME}" -ge "${RUN_START_EPOCH}" ]]; then
        echo "[WARN] MuJoCo GLFW viewer aborted during shutdown after metrics were written; treating rollout as complete." >&2
    else
        exit "${PYTHON_STATUS}"
    fi
fi
