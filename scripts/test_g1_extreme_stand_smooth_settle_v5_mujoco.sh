...
BASELINE_TARGET_LIMITER_ENABLE=${BASELINE_TARGET_LIMITER_ENABLE:-False}
...
BASELINE_TARGET_LIMITER_ENABLE=${BASELINE_TARGET_LIMITER_ENABLE}
CANDIDATE_TARGET_LIMITER_ENABLE=${CANDIDATE_TARGET_LIMITER_ENABLE}
EOF
...
run_model() {
    local label=$1 checkpoint=$2 checkpoint_sha=$3 target_limiter_enable=${4:-False}
...
run_model \
    "${BASELINE_LABEL}" "${V4_CHECKPOINT}" "${actual_v4_sha}" \
    "${BASELINE_TARGET_LIMITER_ENABLE}"
run_model \
    "${CANDIDATE_LABEL}" "${V5_CHECKPOINT}" "${actual_v5_sha}" \
